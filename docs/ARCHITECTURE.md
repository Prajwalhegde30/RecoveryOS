...
**Status:** Proposed baseline — implementation through Phase 15 foundation; architecture review pending
**Owner:** RecoveryOS engineering  
**Product source of truth:** [PRD.md](./PRD.md)  
**Decision record:** [DECISIONS.md](./DECISIONS.md)  
**Data model:** [DATA_MODEL.md](./DATA_MODEL.md)

**Implementation status:** The repository baseline, SQLAlchemy persistence metadata, Alembic migration runtime, initial schema migrations, local PostgreSQL Compose service, persistence constraint tests, canonical event contract, signed webhook boundary, event idempotency, safe replay handling, source-aware obligation identity, one-case-per-obligation association, PRD-aligned audited state transitions, deterministic root-cause diagnosis, configurable integer-safe case scoring, provider-neutral schema-validated AI recommendations, deterministic fallback orchestration, fallback audit provenance, typed versioned merchant-policy lifecycle, deterministic policy precedence evaluation, audited approval resolution, durable jobs, policy-trace persistence, a provider-independent restart-safe worker core, SDK-neutral payment/messaging transport adapters, deterministic simulated payment/messaging providers, registered-action routing, a seeded simulator event runner, authoritative provider-confirmed payment reconciliation, refund/reversal adjustment handling, server-side opt-out enforcement, configurable incident detection, incident evidence snapshots, case-level incident suppression, safe pending-job cancellation, cooldown release, MVP case-level attribution, persisted-fact recovery metrics, versioned typed read API routes, server-generated dashboard freshness metadata, a bounded tenant-scoped audit read, a tenant-scoped seeded simulator start route, a responsive shared-UI dashboard shell, local signed bearer-token validation, production-mode JWKS verification, membership-backed effective roles, Admin gating on the simulator route, bounded high-risk request limiting, correlation-response metadata, persisted worker heartbeat health, database-backed readiness, safe database-degraded operational health, and persisted provider execution latency dimensions are implemented. Future manual retry/intervention/configuration mutation operations, complete aggregate isolation verification, broader dashboard/policy workflows, deployment-backed alerting, and deeper structured observability remain planned for later phases; experiment/control-treatment analytics remain optional/later work.

The latest completed API operations include authenticated Operator/Admin action requests, approval resolution, and a bounded Operator/Admin audit read. Action requests persist the deterministic policy decision and effective policy version, then schedule a durable idempotent job; provider execution remains worker-owned. Simulator lifecycle start/status/reset persists tenant-scoped run identity without deleting generated facts. Production mode validates asymmetric JWTs through a configured JWKS endpoint; local mode remains an explicitly selected HMAC demo path. Broader aggregate isolation verification, future mutation coverage, and detailed dashboard operations remain planned.
